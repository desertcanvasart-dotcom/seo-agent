...
export default async function ResearchPage() {
  ...
  return (
    <div>
      ...
      <p className="text-sm text-[var(--text-secondary)] mb-6">Analyze competitors and find content gaps</p>
                ...
                <div key={j.id} className="flex items-center justify-between p-3 bg-[var(--bg)] rounded-lg">
                  ...
                  <div className="flex items-center gap-3">
                    <span className={`badge ${
                      ...
                      j.status === "running" ? "badge-yellow" :
                      j.status === "failed" ? "badge-red" : "badge-gray"
                    }`}>{j.status}</span>
              ...
              {(gaps.gaps as any[]).slice(0, 25).map((g: any, i: number) => (
                <tr key={i}>
                  ...
                  <td style={{ textAlign: "right" }}>
                    <span className="font-mono text-sm">{g.competitor_coverage}</span>
                  </td>
                  ...
                  <td style={{ textAlign: "right" }}>
                    <span className={`font-mono font-medium ${
                      g.opportunity_score >= 60 ? "text-[var(--red)]" :
                      g.opportunity_score >= 40 ? "text-[var(--yellow)]" : "text-[var(--text-muted)]"
                    }`}>{g.opportunity_score}</span>
                  </td>
                </tr>
              ))}
            </tbody>
